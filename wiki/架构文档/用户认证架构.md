...
graph TB
    ...
    subgraph "Domain Layer"
        ...
        BiometricService[BiometricService<br/>生物识别服务]
    end
    ...
    subgraph "Platform Layer"
        LocalAuth[local_auth<br/>生物识别插件]
...
| 方案 | 优点 | 缺点 | 决策 |
|------|------|------|------|
| **平台 Keychain** | 硬件安全、标准做法 | 平台依赖 | ✅ 采用 |
| 应用内存储 | 跨平台一致 | 安全性低 | ❌ 不采用 |
| 自定义实现 | 可控 | 复杂、易出错 | ❌ 不采用 |

**决策理由**：使用平台原生安全存储是最佳实践。
...
| biometric_fallback_rate | 比率 | 生物识别失败回退比例 |
