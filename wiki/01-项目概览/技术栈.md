...
| **auth** | 主密码验证、生物识别 | AuthService, BiometricService |
...
| 平台 | 支持 | 特性 |
|------|------|------|
| iOS | ✅ | Face ID / Touch ID、AutoFill |
| Android | ✅ | 指纹、Autofill、FLAG_SECURE |
...
class BiometricPlatform {
  static Future<bool> authenticate() async {
    if (Platform.isIOS) {
      return await _iosBiometric.authenticate();
    } else if (Platform.isAndroid) {
      return await _androidBiometric.authenticate();
    } else if (Platform.isWindows) {
      return await _windowsHello.authenticate();
    }
    return false;
  }
}
