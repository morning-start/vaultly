...
> 需求文档 - 定义用户认证的数据模型、数据流动、状态管理、接口设计
...
class VaultMetadata {
  ...
  // 状态信息
  final bool isBiometricEnabled;      // 是否启用生物识别
  final String? biometricKeyAlias;    // 生物识别密钥别名
...
class AuthSession {
  ...
  final bool isBiometric;             // 是否通过生物识别
}
...
| isBiometricEnabled | bool | - | ✅ | 默认 false |
| biometricKeyAlias | String | - | ❌ | 平台密钥库中的别名 |
...
| 已锁定 | UNLOCK_BIOMETRIC | 生物识别中 | 调用生物识别 |
| 生物识别中 | BIOMETRIC_SUCCESS | 已解锁 | 加载保险库 |
| 已解锁 | LOCK | 已锁定 | 清除内存密钥 |
...
enum AuthResult {
  ...
  failure,           // 认证失败
  lockedOut,         // 锁定（多次失败）
  biometricsNotAvailable,  // 生物识别不可用
  biometricsNotEnabled,    // 生物识别未启用
  error,             // 系统错误
}
