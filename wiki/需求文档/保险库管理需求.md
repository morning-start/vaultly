...
class VaultService {
  ...
  Future<VaultEntry> addEntry(VaultEntry entry) async {
    ...
    // 保存到仓库
    return await _repository.saveEntry(
      entry.copyWith(encryptedData: encryptedData),
    );
  }
  ...
  Future<VaultEntry?> getEntry(String id) async {
    ...
    // 解密敏感字段
    final decryptedData = await _cryptoService.decrypt(
      entry.encryptedData,
      await _getVaultKey(),
    );
    
    return _mergeEntryData(entry, jsonDecode(decryptedData));
  }
}
```
...
| 文档概念 | 实现文件 | 说明 |
|----------|----------|------|
| **VaultEntry** | `lib/core/models/vault_entry.dart` | 条目基类及子类 |
| **VaultService** | `lib/core/services/vault_service.dart` | 业务逻辑实现 |
| **VaultRepository** | `lib/core/repositories/vault_repository.dart` | 数据访问层 |
| **EncryptedData** | `lib/core/crypto/services/crypto_service.dart` | 加密数据结构 |
| **状态管理** | `lib/core/providers/vault_service_provider.dart` | Riverpod Providers |
